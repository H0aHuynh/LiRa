...
kptr_t kproc_find_by_name(char* nm)
{
    uint64_t proc = g_exp.kernel_proc;
    
    while (true) {
        uint64_t nameptr = proc + OFFSET(proc, p_name);
        char name[32];
        kapi_read(nameptr, &name, 32);
        if(strcmp(name, nm) == 0) {
            return proc;
        }
        proc = kapi_read64(proc + OFFSET(proc, le_prev));
    }
    return 0;
}

int pid_by_name(char* nm) {
    return kapi_read32(kproc_find_by_name(nm) + OFFSET(proc, p_pid));
}
